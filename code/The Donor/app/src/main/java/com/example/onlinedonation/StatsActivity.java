...
public class StatsActivity extends AppCompatActivity {
    ...
    int userDonatedFood;
    int userDonatedClothes;
    int userDonatedBooks;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        firebaseFirestore.collection("UserDonationBooks").get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
            @Override
            public void onComplete(@NonNull Task<QuerySnapshot> task) {

                if(task.isSuccessful())
                {
                    userDonatedBooks = task.getResult().size();

                    firebaseFirestore.collection("UserDonationFood").get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                        @Override
                        public void onComplete(@NonNull Task<QuerySnapshot> task) {

                            if(task.isSuccessful())
                            {
                                userDonatedFood = task.getResult().size();



                                firebaseFirestore.collection("UserDonationClothes").get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                                    @Override
                                    public void onComplete(@NonNull Task<QuerySnapshot> task) {

                                        if(task.isSuccessful())
                                        {
                                            userDonatedClothes = task.getResult().size();


                                            List<PieEntry> entries = new ArrayList<>();

                                            entries.add(new PieEntry(Float.parseFloat(String.valueOf(userDonatedBooks)), "Books"));
                                            entries.add(new PieEntry(Float.parseFloat(String.valueOf(userDonatedClothes)), "Clothes"));
                                            entries.add(new PieEntry(Float.parseFloat(String.valueOf(userDonatedFood)), "Food"));


                                            PieDataSet set = new PieDataSet(entries, "Total Ngo Donatios");

                                            pieChart2.setCenterText("User Donations");
                                            PieData data = new PieData(set);
